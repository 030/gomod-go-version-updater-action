...
def determine_whether_go_version_in_go_mod_file_contains_patch_version() -> bool:
    ...
    logging.info(f"current golang version that is defined in the go.mod: {version}")
...
def get_latest_go_version():
    ...
    try:
        ...
        logging.info(f"latest go version: {latest_version} according to: {GO_VERSIONS_URL}")
...
def regex_replace_go_version_in_go_mod_file(replacement):
    try:
        ...
        logging.info(
            f"Pattern '{GO_MOD_GO_VERSION_REGEX}' will be replaced in go.mod by: {replacement}")
    except FileNotFoundError:
